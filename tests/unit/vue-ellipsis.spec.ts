...
const getWrapper = (options?: MountingOptions<any>) => mount(VueEllipsis3, options);
...
describe('vue-ellipsis.vue', () => {
  ...
  it('[native-ellipsis] ellipsis 是否生效', () => {
    ...
    expect('element' in wrapper.find('.ellipsis')).toBeFalsy();
  });
  ...
  it('[js-ellipsis] useInnerHtml 是否生效', () => {
    ...
    expect(wrapper.find('.vue-ellipsis-js-content').text()).toBe(
      '这是一段非常非常非常非常非常非常非常非常非常非常非常非常非常长的文本'
    );
  });
